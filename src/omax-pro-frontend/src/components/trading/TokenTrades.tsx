...
function parseTradePrice(apiValue: number, btcPriceUSD: number) {
  // API price format: satoshis * 1000 according to the useOdinAPI format
                        ...
                        <TableCell className="text-right font-mono">
                          <div>
                            <div className="font-medium">
                              ...
                              {displayCurrency === 'sats' && `${(trade.amount_btc / SATOSHI_TO_BTC).toLocaleString()} sats`}
                            </div>
                            <div className="text-xs text-muted-foreground">
                              {displayCurrency !== 'btc' ? `${trade.amount_btc.toFixed(8)} BTC` : amountData.formatted.usd}
                            </div>
                          </div>
                        </TableCell>
                        <TableCell className="text-right font-mono">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t">
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                ...
                <div className="text-lg font-bold text-success">
                  {trades.filter(t => t.buy).reduce((sum, t) => sum + t.amount_btc, 0).toFixed(6)} BTC
                </div>
              </div>
              <div className="text-center p-3 bg-muted/50 rounded-lg">
              ...
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <div className="text-sm text-muted-foreground">Volume (USD)</div>
                <div className="text-lg font-bold">
                  ${(trades.reduce((sum, t) => sum + t.amount_btc, 0) * btcPriceUSD).toLocaleString()}
                </div>
              </div>
            </div>
          </div>
