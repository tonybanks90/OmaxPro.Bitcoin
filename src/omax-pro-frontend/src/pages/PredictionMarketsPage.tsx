...
import { usePredictionMarkets } from '../hooks/usePredictionMarketsAPI';
...
export default function PredictionMarketsPage() {
  ...
  const filteredAndSortedMarkets = markets
    .filter(market => {
      const matchesCategory = activeCategory === 'all' || market.category.toLowerCase() === activeCategory.toLowerCase();
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
          <div className="bg-surface border border-border rounded-lg p-4">
            ...
            <div className="font-bold text-lg text-foreground">
              {statsLoading ? <span className="animate-pulse bg-muted h-6 w-20 block rounded" /> : globalStats?.totalVolumeUSD ?? '$0.00'}
            </div>
          </div>
          ...
          <div className="bg-surface border border-border rounded-lg p-4">
            ...
            <div className="font-bold text-lg text-foreground">
              {statsLoading ? <span className="animate-pulse bg-muted h-6 w-12 block rounded" /> : globalStats?.activeMarkets ?? 0}
            </div>
          </div>
          ...
          <div className="bg-surface border border-border rounded-lg p-4">
            ...
            <div className="font-bold text-lg text-foreground">
              {statsLoading ? <span className="animate-pulse bg-muted h-6 w-16 block rounded" /> : globalStats?.totalParticipants.toLocaleString() ?? 0}
            </div>
          </div>
          ...
          <div className="bg-surface border border-border rounded-lg p-4">
            ...
            <div className="font-bold text-lg text-foreground">
              {statsLoading ? <span className="animate-pulse bg-muted h-6 w-8 block rounded" /> : globalStats?.resolvingToday ?? 0}
            </div>
          </div>
        </div>
