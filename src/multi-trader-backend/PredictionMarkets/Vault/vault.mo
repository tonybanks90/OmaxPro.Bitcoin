...
actor Vault {
    ...
    public shared(msg) func registerMarketWithValidation(
        ...
        Debug.print("Registered " # debug_show(marketType) # " market " # Nat.toText(marketId) # 
...
public shared(msg) func setupMarketVault(
    ...
    marketStats.put(request.marketId, {
        totalDeposited = 0;
        totalWithdrawn = 0;
        transactionCount = 0;
        lastActivity = ?timestamp;
    ...
    Debug.print("Setup vault for market " # Nat.toText(request.marketId) # 
                        ...
                        try {
                            switch (await ledgerActor.icrc2_transfer_from(transferArgs)) {
                                case (#Ok(blockIndex)) {
                                    ...
                                    Debug.print("Pulled " # Nat.toText(amount) # " ckBTC from " # Principal.toText(user) # 
                                ...
                                case (#Err(error)) {
                                    let errorMsg = switch (error) {
                                        ...
                                        case (#Duplicate(details)) { "Duplicate transaction: " # Nat.toText(details.duplicate_of) };
                        ...
                        try {
                            switch (await ledgerActor.icrc1_transfer(transferArgs)) {
                                case (#Ok(blockIndex)) {
                                    ...
                                    Debug.print("Paid " # Nat.toText(amount) # " ckBTC from market " # Nat.toText(marketId) # 
        ...
        switch (marketAccounts.get(marketId)) {
            ...
            case (?subaccount) {
                let stats = switch (marketStats.get(marketId)) {
                    case (?s) { s };
                    case (null) { 
                        { totalDeposited = 0; totalWithdrawn = 0; transactionCount = 0; lastActivity = null }
                    };
                };
    ...
    public query func validateMarket(marketId: MarketId) : async Result.Result<{
        exists: Bool;
        active: Bool;
        marketType: ?MarketType;
        subaccount: ?[Nat8];
    ...
    public func checkUserAllowance(user: Principal, amount: Nat) : async Result.Result<{
        allowance: Nat;
        sufficient: Bool;
        required: Nat;
    }, Text> {
