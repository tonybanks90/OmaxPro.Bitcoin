...
actor Markets {
    ...
    private func selectVaultAddress(
        ...
        switch (marketType, vaultConfig, tokenIdentifier) {
            // Binary market: single vault for all trades
            case (#Binary, #Binary(config), _) {
                config.marketVault
            };
            
            // Multiple choice: single vault for all outcomes
            case (#MultipleChoice, #MultipleChoice(config), _) {
                config.marketVault
            };
            
            // Compound: route to subject-specific vault
            case (#Compound, #Compound(config), #Subject((subjectName, _))) {
                switch (Array.find<(Text, Principal)>(config.subjectVaults, func((name, _)) = name == subjectName)) {
                    case (null) { 
    ...
    public shared(msg) func registerMultipleChoiceMarketWithVault(
        ...
        if (not isUniqueArray(outcomeNames)) {
        ...
        if (not isUniqueArray(ledgerPrincipals)) {
    ...
    public shared(msg) func registerCompoundMarketWithVault(
        ...
        if (not isUniqueArray(subjectNames)) {
        ...
        if (not isUniqueArray(allLedgers)) {
